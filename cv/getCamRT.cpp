...
int caculateCamRT(cv::Mat im, vector<cv::Point2d> Points2D)
{
	...
	//cameraMatrix_result = camera_matrix;
	//distCoeffs_result = distortion_coefficients;
	...
	for (int i = 0; i < 4; i++) {
		...
		if (!getline(points_fin, p3d_str)) {
			points_fin.close();
			cout << "At least 4 points needed!!!";
			return -1;
		}
...
//初始化输出矩阵
	...
	//solvePnP(Points3D, Points2D, camera_matrix, distortion_coefficients, rvec, tvec, false, CV_ITERATIVE);			//Gao的方法可以使用任意四个特征点，特征点数量不能少于4也不能多于4
	...
	//cout << "Distance TO (0,0,0)" << sqrt(Cx*Cx + Cy*Cy + Cz*Cz) << endl;

	//fout2.close();

	cv::Mat R_w2c, r_w2c, t_w2c;
	cv::solvePnP(Points3D, Points2D, camera_matrix, distortion_coefficients, r_w2c, t_w2c, false, cv::SOLVEPNP_P3P);
	...
	fout << eulerAngle2 * 180 / 3.1415926 << endl;
	ofstream fout2("./pnp_t.txt");

	double Cx = t_c2w.ptr<double>(0)[0];
	double Cy = t_c2w.ptr<double>(0)[1];
	double Cz = t_c2w.ptr<double>(0)[2];
	...
	cv::projectPoints(Points3D, r_w2c, t_w2c, camera_matrix, distortion_coefficients, projectedPoints);
	...
	Mat imToShow;
	resize(im, imToShow, Size(im.cols / 2, im.rows / 2));

	cv::imshow("Out", imToShow);
	return 0;
}
